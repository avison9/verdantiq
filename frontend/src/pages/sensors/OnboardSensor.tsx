...
const SensorBeacon = ({ d = 0 }: { d?: number }) => (
  <div className="relative flex items-center justify-center w-full h-full select-none pointer-events-none">
    ...
    <div className="absolute w-36 h-36 rounded-full bg-emerald-400/15 animate-ping"
      style={{ animationDuration: "2.5s", animationDelay: `${d + 0.3}s` }} />
      ...
      <div className="flex flex-col items-center gap-1 opacity-60">
        {[0, 1, 2].map(i => (
          <div key={i} className="w-0.5 h-3 bg-emerald-500 rounded-full animate-pulse"
            style={{ animationDelay: `${d + i * 0.2}s`, opacity: 1 - i * 0.25 }} />
        ))}
      </div>
...
const OnboardSensor = () => {
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="flex gap-8 items-stretch" style={{ minHeight: "calc(100vh - 160px)" }}>
        {/* Left: form */}
