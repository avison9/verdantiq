...
const FarmEdit = () => {
  usePageTitle("Edit Farm — VerdantIQ");
  const { farmId } = useParams<{ farmId: string }>();
  const navigate = useNavigate();

  const { data: farm, isLoading } = useGetFarmQuery(farmId!, { skip: !farmId });
  const [updateFarm, { isLoading: saving }] = useUpdateFarmMutation();
  ...
  // Pre-populate form once farm data loads
  useEffect(() => {
    ...
    setForm({
      ...
      farm_size_ha:     farm.farm_size_ha != null ? String(farm.farm_size_ha) : "",
      farm_type:        farm.farm_type ?? "",
      latitude:         farm.latitude != null ? String(farm.latitude) : "",
      longitude:        farm.longitude != null ? String(farm.longitude) : "",
      perimeter_km:     farm.perimeter_km != null ? String(farm.perimeter_km) : "",
      soil_type:        farm.soil_type ?? "",
      rainfall_avg_mm:  farm.rainfall_avg_mm != null ? String(farm.rainfall_avg_mm) : "",
      sunlight_avg_hrs: farm.sunlight_avg_hrs != null ? String(farm.sunlight_avg_hrs) : "",
      notes:            farm.notes ?? "",
    });
    setSelectedCrops(farm.crops ?? []);
    setCropHistory(
      (farm.crop_history ?? []).map(h => ({
        year:       String((h as { year?: unknown }).year ?? ""),
        crops:      Array.isArray((h as { crops?: unknown }).crops)
                      ? ((h as { crops: string[] }).crops).join(", ")
                      : String((h as { crops?: unknown }).crops ?? ""),
        yield_tons: (h as { yield_tons?: unknown }).yield_tons != null
                      ? String((h as { yield_tons: number }).yield_tons)
                      : "",
      }))
    );
    setReady(true);
  ...
  const addCustomCrop = () => {
    ...
    if (t && !selectedCrops.includes(t)) setSelectedCrops(prev => [...prev, t]);
    setCustomCrop("");
  };
    ...
    try {
      ...
      const history = cropHistory
        ...
        .map(r => ({
          ...
          crops:      r.crops.split(",").map(c => c.trim()),
          yield_tons: r.yield_tons ? parseFloat(r.yield_tons) : undefined,
        }));
      ...
      toast.error((err as { data?: { detail?: string } })?.data?.detail ?? "Failed to update farm");
    }
  };
  ...
  return (
    <div className="px-6 py-8">
      <div className="mb-8">
        <button onClick={() => navigate(`/farm/${farmId}`)}
        ...
        <h1 className="text-lg font-semibold text-gray-800">Edit Farm</h1>
        <p className="text-sm text-gray-400 mt-0.5">Update the details for {farm.farm_name}.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-3">
          ...
          {cropHistory.length === 0 && (
            <p className="text-xs text-gray-400">No crop history recorded yet.</p>
          )}
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate(`/farm/${farmId}`)}
